...
void move_player(t_game *game)
{
    ...
    if (game->keys[119]) // W
    {
        if (game->cfg.map[(int)game->player.pos_y][(int)(game->player.pos_x + game->player.dir_x * MOVE_SPEED)] != '1')
            game->player.pos_x += game->player.dir_x * MOVE_SPEED;
        if (game->cfg.map[(int)(game->player.pos_y + game->player.dir_y * MOVE_SPEED)][(int)game->player.pos_x] != '1')
    ...
    if (game->keys[100]) // D tuşu: Sağa kayma
    {
        // Yön vektörüne dik olan (-dir_y, dir_x) vektörünü kullanıyoruz
        if (game->cfg.map[(int)game->player.pos_y][(int)(game->player.pos_x - game->player.dir_y * MOVE_SPEED)] != '1')
            game->player.pos_x -= game->player.dir_y * MOVE_SPEED;
        if (game->cfg.map[(int)(game->player.pos_y + game->player.dir_x * MOVE_SPEED)][(int)game->player.pos_x] != '1')
            game->player.pos_y += game->player.dir_x * MOVE_SPEED;
    }
    if (game->keys[97]) // A tuşu: Sola kayma
    {
        // Yön vektörüne dik olan (dir_y, -dir_x) vektörünü kullanıyoruz
        if (game->cfg.map[(int)game->player.pos_y][(int)(game->player.pos_x + game->player.dir_y * MOVE_SPEED)] != '1')
            game->player.pos_x += game->player.dir_y * MOVE_SPEED;
    ...
    if (game->keys[65363]) // Sağ Ok Tuşu: Sağa dön (Saat yönü)
    {
    ...
    if (game->keys[65361]) // Sol Ok Tuşu: Sola dön (Saat yönünün tersi)
    {
        old_dir_x = game->player.dir_x;
